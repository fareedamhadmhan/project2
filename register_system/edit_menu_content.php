...
if (!empty($name_menu)) {
    ...
    $sql_file   = "SELECT * FROM attach_file WHERE menu_name='$name_menu'" ;
    $result         = @$con->query($sql);
    $result_file    = @$con->query($sql_file);
    $row            = $result->fetch_assoc();
    $row_file       = $result_file->fetch_assoc();
    $path           = $row_file["path_file"];
    ...
    if ($row_nums >= 1) {
        echo "<form id='data1' action='' method='post' enctype='multipart/form-data'>
        <input type='hidden' name='menu_name' value='" . $name_menu . "'>
        <table>
            <tr>
                <td align='center'><img width='100' height='100' src='../register_system/".$path."'></td>
            </tr>  
            <tr><td><br></td></tr>   
            <tr>
